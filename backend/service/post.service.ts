...
import {
  ...
  returnPost,
  returnPosts,
} from "../types/service/InterfacePost";
...
const getLikeItPost = async (likeItData: {
  postUuid: string;
}): Promise<returnGetPostLikeIt> => {
  try {
...
const createPost = async (postData: IPost): Promise<returnPost> => {
  const { content, id, tags, imageUri } = postData;
...
const deletePost_service = async ({
  postUuid,
  id,
}: {
  postUuid: string;
...
const getPostsSortByTime = async ({
  limit = "150",
}: {
  limit: string;
}): Promise<returnPosts> => {
...
const getHashTagPosts_service = async ({
  tag,
}: {
  tag: string;
}): Promise<returnPosts> => {
