...
import {
  ...
  INonMemberComment,
  returnComment,
} from "../types/service/InterfaceComment";
import { getRepository } from "typeorm";

const createMemberComment = async (
  commentData: IMemberComment
): Promise<returnComment> => {
  const { content, postUuid, id, parentUuid } = commentData;
  try {
...
const createNonMemberComment = async (
  commentData: INonMemberComment
): Promise<returnComment> => {
  const { content, postUuid, anonymouseId, password, parentUuid } = commentData;
  try {
    const postFromUuid = await Post.findOneOrFail({ uuid: postUuid });
    let comment: any;
    if (parentUuid !== undefined) {
...
const deleteNonMemberCommentFromUuid = async (data: any) => {
  const { commentPassword, commentUuid } = data;
  try {
...
const getCommentsFromPostUuid = async ({
  postUuid,
}: any): Promise<returnComment> => {
  try {
