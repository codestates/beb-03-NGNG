  ...
  return (
    <>
      <Card sx={{ backgroundColor: "#9F9C92", display: "flex", mb: 1 }}>
        <Box>
          <CardHeader
            avatar={<Avatar aria-label="recipe" src={imageUri}></Avatar>}
      ...
      {childComments.length > 0 && (
        <Box sx={{ display: "flex" }}>
          <FontAwesomeIcon
            icon={faCaretRight}
            fontSize={"60px"}
            style={{ padding: "14px", paddingRight: "20px", color: "#3E3E41" }}
          />
          <Box sx={{ flex: 1 }}>
            {childComments.map((comment) => {
              return (
                <SubComment
                  ...
                  anonymouseId={comment.annonymouseId}
                  userId={comment.id}
